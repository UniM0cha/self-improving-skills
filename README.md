...
| `SIS_DISTILLER_MODEL` | `opus` | Model of the distillation child. **This plugin's children run at Opus or below** (0.18.2): unset means `opus`, a lower tier such as `sonnet` passes through, a higher one (`fable`, `mythos`) is lowered to `opus` and noted in the job summary |
| `SIS_CURATE_MODEL` | `opus` | Model of the consolidation/compression children; same Opus-or-below rule |
