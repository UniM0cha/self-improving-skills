...
4. **use_count 는 한 방향으로만 증거입니다.** 세션 스킬 목록은 예산을 넘기면 덜 쓰는 스킬부터 설명을 떼어 내므로, 설명이 잘린 스킬은 애초에 호출될 수 없었습니다 — `use=0` 은 가치에 대해 아무 말도 하지 않습니다. 반대로 `use>0` 은 그 스킬이 지금 모양대로 작동한다는 증거이니, 통합할 때 그쪽을 umbrella 기준으로 삼으세요. 겹침 판단은 빈도가 아니라 **내용**으로 합니다. (미사용 기반 정리는 시간 기반 상태머신이 이미 담당합니다 — 이 패스의 일이 아닙니다.)
...
학습 스킬의 description은 **모든 세션의 스킬 목록에 실리고, 목록은 예산을 넘기면 덜 쓰는 스킬부터 설명을 떼어 냅니다**. 300자를 넘는 description은 트리거 상황 하나를 지목하는 한 문장으로 압축하세요. umbrella 의 description 도 같습니다 — 흡수한 스킬들의 트리거 문구를 이어 붙이지 마세요(0.17.0 의 자동 통합이 1,024자를 넘겨 검증기에 걸린 실제 실패 모드입니다). 한 문장에 안 들어가면 그 묶음은 한 스킬이 아닙니다.
