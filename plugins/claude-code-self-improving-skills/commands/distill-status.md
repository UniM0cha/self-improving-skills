...
- `unprotected_write` — 스킬 트리 안에서 가드가 롤백을 보장할 수 없는 변경이 있었습니다.
  **재시도 전에 경로를 사용자와 함께 확인하세요.** 이건 안전장치가 실제로 발동한 경우입니다.
- `out_of_scope_write` — **없어진 검사입니다.** 0.17.0 까지는 홈의 파일 12개
  (`~/.claude/settings.json`, 셸 rc 등)가 실행 중 바뀌면 작업을 막았는데, CLI 자체가
  `settings.json` 을 평소 동작으로 다시 써서 멀쩡한 작업이 막혔습니다. 0.18.0 부터
  스킬 트리 밖은 관측하지 않습니다(플러그인 소유자 결정). 남아 있는 작업은 그대로 재시도하면 됩니다.
...
단 `unprotected_write` 가 섞여 있으면 **먼저 사용자에게 확인**한 뒤 재시도하세요.
