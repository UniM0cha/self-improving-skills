...
> **Claude Code를 위한 Hermes Agent식 자기개선 루프.**
>
...
Claude Code 버전에 따라 명령 형태가 다르면, 플러그인 UI에서 `https://github.com/UniM0cha/claude-self-improving-skills`를 마켓플레이스로 추가하고 `self-improving-skills` 플러그인을 설치하면 됩니다.
...
작업이 복잡했는데 아직 증류 안 됐으면 1회성 block 반환
  ↓
Claude가 self-improving-skills:skill-distiller에 위임 (백그라운드)
  ↓
...
```text
.claude-plugin/marketplace.json          # 단일 플러그인 Claude Code 마켓플레이스 매니페스트
