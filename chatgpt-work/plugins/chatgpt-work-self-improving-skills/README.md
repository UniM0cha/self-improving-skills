...
2. ChatGPT 데스크톱 앱을 재시작하고 Work 모드의 Plugins에서 `Self-Improving Skills` → `chatgpt-work-self-improving-skills`를 설치합니다. 현행 데스크톱 앱에서는 Codex 변형도 같은 marketplace에 함께 보이므로 이름을 확인해 Work 변형을 선택하세요.
